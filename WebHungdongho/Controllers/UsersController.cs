...
namespace WebHungdongho.Controllers
{
    public class UsersController : Controller
    {
        ...
        {
            var list = db.Users.Where(x => x.mod == 2).ToList();
            return View(list);
        }
        [HttpPost]
        public ActionResult Index(string key)
        {
            ...
            x.TenDangNhap.Contains(key) || x.MatKhau.Contains(key) || x.Email.Contains(key) || x.SoDienThoai.Contains(key)) && x.mod == 2).ToList();
            if (key == "")
            {
        ...
        [HttpPost]
        public ActionResult Resetpassword(string tendangnhap)
        {
           
            if (!string.IsNullOrEmpty(tendangnhap))
            {
               
                var user = db.Users.SingleOrDefault(u => u.TenDangNhap == tendangnhap);

                if (user != null)
                {
                  
                    user.MatKhau = HashPassword("111111") ;

                    db.SaveChanges();

                    ViewBag.Message = "Mật khẩu đã được reset thành 111111";
                    return View();
                }
                else
                {
            ...
            else
            {
                ...
                ViewBag.Message = "Vui lòng nhập tên đăng nhập.";
            }
